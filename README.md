...
_Processes cryptocurrency data to compute average price changes and the most recent prices, then writes the results to a CSV file:_
  ...
  # Group by 'Name' and calculate the average of '24h Change', rename the column to 'AVG price change'
  result_df = filtered_df.groupBy("Name").agg(
      avg("`24h Change`").alias("AVG price change"),
      max("`Current Price`")  # Keep the most recent 'Current Price'
  )

  # Drop the 'Creation Date' column
  result_df = result_df.drop("Creation Date")
  ...
  csv_file = dbutils.fs.ls(output_path)[0].path
  dbutils.fs.mv(csv_file, "dbfs:/tmp/result_df.csv")

  # Provide a link to download the CSV file
  print("The prices are in USD.")
  print("The 'AVG price change' is the result of averaging all the daily price changes.")
  print("Download the result CSV file from the following link:")
  print("https://adb-3022457162513861.1.azuredatabricks.net/FileStore/result_df.csv")
  print()
  print("Note: this link will only be available while the cluster is active.")
  ```
...
6. _Download the file to your local `data` directory (where all the files generated by `CryptoETL.java` are stored):_
    ```bash
...
1. **Downloading the CSV file from Databricks**
...
2. **Cluster Node Availability Issues**
   ...
   _If you encounter node availability issues when setting up your Databricks cluster, refer to the Azure documentation and run the necessary commands to list available nodes._
	 ...
	 _If you want to see the documentation, see this_ [link](https://learn.microsoft.com/en-us/azure/azure-resource-manager/troubleshooting/error-sku-not-available?tabs=azure-cli#code-try-4) ⬅️
